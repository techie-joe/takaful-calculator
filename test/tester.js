(function (
  ...
  function err(err) { return c.error(`Tester Error: ${err}`) }
  ...
  w.addEventListener('load', function (event) {
    ...
    // log(tester,load_tester_to);

    var e;

    if (w.load_tester_to) {
      log(`load_tester_to: id: ${w.load_tester_to}`);
      e = d.getElementById(w.load_tester_to);
    }

    else if (tester) {
      log(`load_tester_to: dom: `, w.tester);
      e = w.tester;
    }

    if (!e) { return err('Fail to load tester. No DOM found.'); }

    log(`routeStart : ${routeStart}`);
    // m.route(e, routeStart, route);
    m.mount(e, route[routeStart]);
